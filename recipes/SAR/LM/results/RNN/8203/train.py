#!/usr/bin/env python3
"""Recipe for fine-tuning a Language Model 
with SAR domain train transcripts.
...
def dataio_prepare(hparams):
    ...
    train_transcripts = glob.glob(
        os.path.join(data_folder, "train.txt"), recursive=True
    )
    dev_transcripts = glob.glob(
        os.path.join(data_folder, "dev.txt"), recursive=True
    )
    test_transcripts = glob.glob(
        os.path.join(data_folder, "test.txt"), recursive=True
    )
