...
class ASR_Brain(sb.Brain):
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            self.hparams.inference_logger.log_stats(
            ...
            with open(self.hparams.inference_per_file, "w") as w:
                w.write("CTC loss stats:\n")
                self.ctc_metrics.write_stats(w)
                w.write("\nPER stats:\n")
                self.per_metrics.write_stats(w)
                print("CTC and PER stats written to ", self.hparams.inference_per_file)
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.takes(hparams["inference_type"])
    ...
    # 3. Create datasets
    hparams["inference_logger"].log_stats(
        stats_meta={"Inference on: ": hparams["inference_type"]}
    )
    data = {}
    data_info = {
...
if __name__ == "__main__":
    ...
    if not "pretrained" in hparams:
        sys.exit('Pre-trained model not found.')
