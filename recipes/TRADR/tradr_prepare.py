...
def prepare_tradr(
    ...
    """
    Prepares the csv files for the TRADR dataset.

    Arguments
    ---------
    data_folder : str
        Path to the folder where the original TRADR dataset is stored.
        This path should include the lang: /datasets/TRADR/en/
    save_folder : str
        The directory where to store the csv files.
    train_tsv_file : str, optional
        Path to the Train TRADR .tsv file (cs)
    dev_tsv_file : str, optional
        Path to the Dev TRADR .tsv file (cs)
    test_tsv_file : str, optional
        Path to the Test TRADR .tsv file (cs)
    ...
    Example
    -------
    >>> from recipes.TRADR.tradr_prepare import prepare_tradr
    >>> data_folder = '/datasets/TRADR/en'
    >>> save_folder = 'exp/TRADR_exp'
    >>> train_tsv_file = '/datasets/TRADR/en/train.tsv'
    >>> dev_tsv_file = '/datasets/TRADR/en/dev.tsv'
    >>> test_tsv_file = '/datasets/TRADR/en/test.tsv'
    >>> accented_letters = False
    ...
    # If not specified point toward standard location w.r.t TRADR tree
    ...
    # Additional checks to make sure the data folder contains TRADR
    check_tradr_folders(data_folder)
...
def skip(save_csv_train, save_csv_dev, save_csv_test):
    """
    Detects if the TRADR data preparation has been already done.
...
def create_csv(
    ...
    ---------
    orig_tsv_file : str
        Path to the TRADR tsv file (standard file).
    data_folder : str
        Path of the TRADR dataset.
    accented_letters : bool, optional
        Defines if accented letters will be kept as individual letters or
        transformed to the closest non-accented letters.
    ...
    for line in tzip(loaded_csv):
        ...
        # Path is at indice 1 in TRADR tsv files. And .mp3 files
...
def check_tradr_folders(data_folder):
    """
    Check if the data folder actually contains the TRADR dataset.
    ...
    ------
    FileNotFoundError
        If data folder doesn't contain TRADR dataset.
    """
    ...
    if not os.path.exists(data_folder + files_str):
        ...
        err_msg = (
            ...
            "the TRADR dataset)" % (data_folder + files_str)
        )
