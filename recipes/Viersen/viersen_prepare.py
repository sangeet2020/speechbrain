...
def prepare_viersen(
    ...
    """
    Prepares the csv files for the Mozilla Viersen dataset.
    Download: https://voice.mozilla.org/en/datasets

    Arguments
    ---------
    data_folder : str
        Path to the folder where the original Viersen dataset is stored.
        This path should include the lang: /datasets/CommonVoice/en/
    save_folder : str
        The directory where to store the csv files.
    train_tsv_file : str, optional
        Path to the Train Viersen .tsv file (cs)
    dev_tsv_file : str, optional
        Path to the Dev Viersen .tsv file (cs)
    test_tsv_file : str, optional
        Path to the Test Viersen .tsv file (cs)
    ...
    Example
    -------
    >>> from recipes.CommonVoice.common_voice_prepare import prepare_viersen
    ...
    # Additional checks to make sure the data folder contains Viersen
    check_viersen_folders(data_folder)
...
def skip(save_csv_train, save_csv_dev, save_csv_test):
    """
    Detects if the Viersen data preparation has been already done.
...
def create_csv(
    ...
    ---------
    orig_tsv_file : str
        Path to the Viersen tsv file (standard file).
    data_folder : str
    ...
    for line in tzip(loaded_csv):
        ...
        # Path is at indice 1 in Viersen tsv files. And .mp3 files
...
def check_viersen_folders(data_folder):
    """
    Check if the data folder actually contains the Viersen dataset.
    ...
    ------
    FileNotFoundError
        If data folder doesn't contain Viersen dataset.
    """
    ...
    if not os.path.exists(data_folder + files_str):
        ...
        err_msg = (
            ...
            "the Viersen dataset)" % (data_folder + files_str)
        )
