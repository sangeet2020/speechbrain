    ...
    def load_transcriptions(self):
        ...
        for csv_file in self.csv_files:
            ...
            for file, seg_id, start, end, text, spkr in zip(files,seg_ids,start_ts,end_ts,texts, spkr_info):
                ...
                f_name = file + "-seg." + str("{0:04}".format(seg_id)) + "-" + str(start) + "-" + str(end) + ".wav"
                
                
                norm_text = self._text_normalization([text]).strip()
                if self.src_dir + "/" + f_name not in self.audio_files:
        ...
        split_list = ['viersen_dataset.tsv','train.tsv', 'dev.tsv', 'test.tsv']
...
def main():
    ...
    viersen = Viersen(args)
    viersen.collect_audio_files()
    viersen.collect_transcriptions()
    viersen.load_transcriptions()
    # viersen.segment_audio()
    
def parse_arguments():
    """ parse arguments """
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("src_dir", help="path to audio dir")
    parser.add_argument("transciption_dir", help="path to csv trans. files")
