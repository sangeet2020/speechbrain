...
Download: https://www1.wdr.de/fernsehen/feuer-und-flamme/index.html
...
def prepare_feuer_flamme(
    ...
    """
    Prepares the csv files for the Feuer-Flamme dataset.
    Download using youtube-dl/MediathekView: 
    https://www1.wdr.de/fernsehen/feuer-und-flamme/index.html

    Arguments
    ---------
    data_folder : str
        Path to the folder where the original Feuer-Flamme 
        dataset is stored: /netscratch/sagar/data/Feuer-Flamme/
    save_folder : str
        The directory where to store the csv files.
    train_tsv_file : str, optional
        Path to the Train Feuer-Flamme .tsv file (cs)
    dev_tsv_file : str, optional
        Path to the Dev Feuer-Flamme .tsv file (cs)
    test_tsv_file : str, optional
        Path to the Test Feuer-Flamme .tsv file (cs)
    ...
    Example
    -------
    >>> from recipes.Feuer-Flamme.common_voice_prepare import prepare_feuer_flamme
    >>> data_folder = '/netscratch/sagar/data/Feuer-Flamme/'
    >>> save_folder = 'exp/Feuer-Flamme_exp'
    >>> train_tsv_file = '/datasets/Feuer-Flamme/train.tsv'
    >>> dev_tsv_file = '/datasets/Feuer-Flamme/dev.tsv'
    >>> test_tsv_file = '/datasets/Feuer-Flamme/test.tsv'
    >>> accented_letters = False
    >>> duration_threshold = 10
    >>> prepare_feuer_flamme( \
                 data_folder, \
                 save_folder, \
                 train_tsv_file, \
                 dev_tsv_file, \
    ...
    # If not specified point toward standard location w.r.t Feuer-Flamme tree
    ...
    # Additional checks to make sure the data folder contains Feuer-Flamme
    check_feuer_flamme_folders(data_folder)
...
def skip(save_csv_train, save_csv_dev, save_csv_test):
    """
    Detects if the Feuer-Flamme data preparation has been already done.
...
def create_csv(
    ...
    ---------
    orig_tsv_file : str
        Path to the Feuer-Flamme tsv file (standard file).
    data_folder : str
        Path of the Feuer-Flamme dataset.
    accented_letters : bool, optional
        Defines if accented letters will be kept as individual letters or
        transformed to the closest non-accented letters.
    ...
    for line in tzip(loaded_csv):
        ...
        # Path is at indice 1 in Feuer-Flamme tsv files. And .mp3 files
...
def check_feuer_flamme_folders(data_folder):
    """
    Check if the data folder actually contains the Feuer-Flamme dataset.
    ...
    ------
    FileNotFoundError
        If data folder doesn't contain Feuer-Flamme dataset.
    """
    ...
    if not os.path.exists(data_folder + files_str):
        ...
        err_msg = (
            ...
            "the Feuer-Flamme dataset)" % (data_folder + files_str)
        )
